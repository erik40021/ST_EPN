...
base_out_dir = "Malig analysis/NMF"; if(!dir.exists(base_out_dir)) dir.create(base_out_dir, r=T)
input_dir = "Malig analysis/NMF/programs by sample"
...
for (s in sample_ids) {
  ...
  names(aggregated_w_matrices)[length(aggregated_w_matrices)] = paste0(s, "_ranks_2-", r)
}
...
intra_min_parameter = 35 # minimal gene overlap between programs of the same sample needed to call them 'robust programs'
...
min_intersect = 8 # minimal number of genes intersecting between robust programs to be aggregated to metaprogram
min_group = 4 # minimal number of (significant) intersections a robust program must have to qualify for metaprogram initiation

# >>>>>>> c) metaprogram evaluation and re-assignment parameters (optional) <<<<<<<
min_intersect_final = 6 # minimal mean intersecting genes between a robust program and all genes of the initial metaprogram programs to be finally assigned to it
...
Sorted_intersection = sort(apply(robust_program_intersects, 2, function(x) (length(which(x>=Min_intersect_initial))-1)), decreasing = TRUE)
MP_programs = list()   ### Every entry contains the robust programs of a chosen metaprogram
MP_genes = list()
k = 1
curr_metaprogram = c()

robust_programs_original = robust_programs # store for later (will change during the loop)
robust_program_intersects_original = robust_program_intersects # same here

while (Sorted_intersection[1] >= Min_group_size) {
  curr_metaprogram = c(curr_metaprogram , names(Sorted_intersection[1]))
...
g = plot_NMF_heatmap(robust_program_intersects_meltI, xlab = paste0("robust NMF program (n = ",length(inds_sorted), ")"))
...
g1 = plot_NMF_heatmap(robust_program_intersects_meltI_NEW, xlab = paste0("All robust NMF programs (n = ",ncol(robust_program_intersects_original), ")"), ylab = "robust NMF program")
...
anno = ggplot(data = color_data, aes(x = 0, y = y)) + geom_tile(aes(fill = Color)) + scale_fill_identity() + theme_void() +
  theme(axis.title = element_blank(), axis.text.y = element_text(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(1,0,1,1), "cm")) + labs(x = NULL, y = NULL) + scale_y_discrete(breaks = color_data$y, labels = color_data$name)
ggsave(anno + g2 + labs(y = NULL) + plot_layout(widths = c(1, 14)), filename = "01_heatmap_MPs_main.png", width = 10.5, height = 8, path = mp_gen_dir)
...
p1 = ggplot(data, aes(x=metaprograms, y=n_percent, fill = sample)) + geom_bar(stat="identity", position="stack") + theme_classic() + theme(legend.key.size = unit(4, "mm")) +
  scale_fill_manual(values = sample_colors) + geom_col() + scale_y_continuous(breaks = c(0, 0.5, 1)) + coord_cartesian(ylim = c(0, 1))
data$patient = sapply(data$sample, function(s) sub("([a-zA-Z].*)", "", s))
data2 = data %>% group_by(metaprograms, patient) %>% dplyr::summarise(n_percent = sum(n_percent)) %>% ungroup()
p2 = ggplot(data2, aes(x=metaprograms, y=n_percent, fill = patient)) + geom_bar(stat="identity", position="stack") + theme_classic() + theme(legend.key.size = unit(4, "mm")) +
  scale_fill_manual(values = sample_colors) + geom_col() + scale_y_continuous(breaks = c(0, 0.5, 1)) + coord_cartesian(ylim = c(0, 1))
ggsave(p1 + p2, filename = "04a_barplot_MPs_by-samples+patients.png", width = 12, height = 4, path = mp_gen_dir)

# ii. composition of MPs regarding subtypes and subtypes_precise
data$subtype = metadata[match(data$sample, metadata$`Study ID`), ]$Subtype
data$subclass = metadata[match(data$sample, metadata$`Study ID`), ]$`Subclass (classifier)`
...
p = ggplot(data, aes(x=sample, y=programs_contributed, fill = metaprograms)) + geom_bar(stat="identity", position="stack") + theme_classic() +
...
# iv. contributions of subtypes and subclass to MPs (biological variables)
subtype_sizes = sapply(unique(data$subtype), FUN = function(x) sum(x == metadata[match(data$sample, metadata$`Study ID`), ]$Subtype, na.rm = T))
subclass_sizes = sapply(unique(data$subclass), FUN = function(x) sum(x == metadata[match(data$sample, metadata$`Study ID`), ]$`Subclass (classifier)`, na.rm = T))
...
data$rel_programs_contributed = data$programs_contributed / subclass_sizes[match(data$subclass, unique(data$subclass))]
...
data$sex = metadata[match(data$sample, metadata$`Study ID`), ]$Sex; data$age = metadata[match(data$sample, metadata$`Study ID`), ]$`Age at resection`
p1 = ggplot(data, aes(x=metaprograms, y=n_percent, fill = sex)) + geom_bar(stat="identity", position="stack") + theme_classic() + theme(legend.key.size = unit(4, "mm")) +
  scale_fill_manual(values = cluster_colors) + geom_col() + scale_y_continuous(breaks = c(0, 0.5, 1)) + coord_cartesian(ylim = c(0, 1))
p2 = ggplot(data, aes(x=metaprograms, y=n_percent, fill = age)) + geom_bar(stat="identity", position="stack") + theme_classic() + theme(legend.key.size = unit(4, "mm")) +
  scale_fill_gradient2(low = "#e1fbfc", mid = "#0b2785", high = "#142252", midpoint = 40) + geom_col() + scale_y_continuous(breaks = c(0, 0.5, 1)) + coord_cartesian(ylim = c(0, 1))
ggsave(p1 + p2, filename = "04e_barplot_MPs_by-sex-and-age.png", width = 6, height = 2.5, path = mp_gen_dir)

# vi. composition of MPs regarding batches (technical variable)
data$batch = metadata[match(data$sample, metadata$`Study ID`), ]$Batch
p1 = ggplot(data, aes(x=metaprograms, y=n_percent, fill = batch)) + geom_bar(stat="identity", position="stack") + theme_classic() + theme(legend.key.size = unit(4, "mm")) +
  scale_fill_manual(values = cluster_colors) + geom_col() + scale_y_continuous(breaks = c(0, 0.5, 1)) + coord_cartesian(ylim = c(0, 1))
ggsave(p1, filename = "04f_barplot_MPs_by-batch.png", width = 3, height = 2.5, path = mp_gen_dir)
...
# a) annotate with GO terms
enrich_signatures_GO(metaprograms, anno_out_dir)
enrich_signatures_GO_differential(metaprograms, anno_out_dir, save_combined = T, save_individually = T, GO_label_width = 50, width = 9, height = 10, x_lab_size = 8)

# b) annotate with reference signatures: Tirosh, Gojo
tirosh_out_dir = file.path(anno_out_dir, "Tirosh ref"); if (!dir.exists(tirosh_out_dir)) dir.create(tirosh_out_dir, r = T)
...
for (i in 1:length(ref_all_tirosh)) { # 1. Tirosh 3CA malignant metaprograms
  ...
  pheatmap::pheatmap(jac_mtx, color = heatmap_col2, angle_col = "90", fontsize = 22, scale = "none", main = paste0("MPs vs. Tirosh ", mps_name, " MPs by jaccard ind."),
                     cluster_cols = F, cluster_rows = F, filename = paste0(tirosh_out_dir, "/01_heatmap_tirosh_", mps_name, "-mps_jaccard_unscaled.png"), width = 12, height = 14)
  pheatmap::pheatmap(jac_mtx, color = heatmap_col1, angle_col = "45", fontsize = 22, scale = "column", main = paste0("MPs vs. Tirosh ", mps_name, " MPs by jaccard ind. (cs)"),
                     cluster_cols = F, cluster_rows = F, filename = paste0(tirosh_out_dir, "/02_heatmap_tirosh_", mps_name, "-mps_jaccard_col-scaled.png"), width = 12, height = 14)
}
# 2. Gojo ST-EPN cluster top 50 DEGs
ref_all_gojo = get_gojo_sigs_all()[1:10] # exclude PF-NSC here (incorrect annotation, actually contains Stress genes, plus different compartment)
jac_mtx = compare_jaccard_ref_vs_sig(metaprograms, ref_all_gojo); jac_mtx = reorder_similarity_matrix_diagonally(jac_mtx, reorder_rows = T)
pheatmap::pheatmap(jac_mtx, color = heatmap_col2, angle_col = "90", fontsize = 22, scale = "none", main = paste0("MPs vs. Gojo top-DEGs by jaccard ind."),
                   cluster_cols = F, cluster_rows = F, filename = file.path(anno_out_dir, "03_heatmap_gojo_top-degs_jaccard_unscaled.png"), width = 12, height = 7)


# ------------------------ part 6: Plot customly ordered final heatmap ----------------------------
new_order = c(1,9, 3,8,5, 4,6,7, 2,11, 10,12,13)
new_order = c(6,7,1,2,3,4,5,8,11,12,9,10,13,14,15) # second reordering (relative to first)
...
anno = ggplot(data = color_data, aes(x = 0, y = y)) + geom_tile(aes(fill = Color)) + scale_fill_identity() + theme_void() +
  theme(axis.title = element_blank(), axis.text.y = element_text(), axis.text.x = element_blank(), axis.ticks.x = element_blank(), plot.margin = unit(c(1,0,1,1), "cm")) + labs(x = NULL, y = NULL) + scale_y_discrete(breaks = color_data$y, labels = color_data$name)
g <- anno + g + plot_layout(widths = c(1, 14))
ggsave(g, filename = "heatmap_MPs_custom_order2.png", width = 10.5, height = 8, path = mp_gen_dir)
...
saveRDS(MP_genes, file.path(mp_gen_dir, "metaprogram_genes.RDS")); saveRDS(MP_programs, file.path(mp_gen_dir, "metaprogram_programs.RDS"))
